...
int distance(t_point from, t_point to, t_point p) {
...
void separate_points(t_point* points, int num_point, t_point from, t_point to, t_point* s1, t_point* s2, int* n1, int* n2) {
    ...
    *n1 = *n2 = 0;
    ...
    for (int i = 0; i < num_point; i++) {
        ...
        else if ((points[i].x - from.x) * dy - (points[i].y - from.y) * dx > 0)
            s2[(*n2)++] = points[i];
    }
}
...
t_line* upper_hull(t_point* points, int num_point, t_point p1, t_point pn, t_line* lines, int* num_line, int* capacity) {
    ...
    t_point* s1 = (t_point*)malloc(num_point * sizeof(t_point));
    assert(s1 != NULL);

    t_point* s2 = (t_point*)malloc(num_point * sizeof(t_point));
    assert(s2 != NULL);

    int n1, n2;

    separate_points(points, num_point, p1, p_max, s1, points, &n1, &num_point);
    separate_points(points, num_point, p_max, pn, s2, points, &n2, &num_point);
