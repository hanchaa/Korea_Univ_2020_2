...
int encoding_binary(char *codes[], FILE *infp, FILE *outfp)
{
	...
	while ((c = getc(infp)) != EOF)
	{
		...
		while (strlen(binaryString) >= 8)
		{
			char out[10] = "";
			strncpy(out, binaryString, 8);
			out[8] = '\0';

			unsigned char binary = strtol(out, 0, 2);
			fwrite(&binary, sizeof(binary), 1, outfp);

			memmove(binaryString, binaryString + 8, strlen(binaryString) - 7);

			cnt += 8;
		}
	}

	int remain = strlen(binaryString);

	if (remain > 0)
	{
		unsigned char binary = strtol(binaryString, 0, 2);
		binary = binary << (8 - remain);
		fwrite(&binary, sizeof(binary), 1, outfp);

		cnt += remain;
	}
	fwrite(&cnt, sizeof(cnt), 1, outfp);
...
{
	int cnt = 0;
	fseek(infp, -4L, SEEK_END);
	fread(&cnt, sizeof(cnt), 1, infp);
	...
	while (cnt >= 8) {
		unsigned char codes;
		fread(&codes, sizeof(codes), 1, infp);
		
		cnt -= 8;

		for (int i = 0; i < 8; i++) {
			unsigned char code = 0x80 & codes;
...
static void _reheapDown(HEAP *heap, int index)
{
	...
	while (1)
	{
		if ((index * 2 + 1) > heap->last)
			return; // leaf node (there is no left subtree)
		...
		if (noright || leftData->freq < rightData->freq)
			largeindex = index * 2 + 1; // left child
		else
			largeindex = index * 2 + 2; // right child
